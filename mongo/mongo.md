...
## Docker
  ...
  - Subir o container, no caso do windows trocar '~' pelo caminho no sistema.
  ```
  docker run --name MongoDB -d -v ~/sample_airbnb:/sample_airbnb mongo
  ```
...
## MongoImport
  ...
  cd sample_airbnb
  mongoimport --db prova --collection airbnb --type json --file listingsAndReviews.json
  ```
...
## MongoSH
  ...
  db.airbnb.insertOne(
    {
      _id: '20000000',
      listing_url: 'https://www.airbnb.com/rooms/20000000',
      name: 'Casa do Messias',
  ...
  - Atualizando o registro inserido
  ```
  db.airbnb.updateOne({_id:"20000000"},{$set:{price:Decimal128("500.00")}})
  ```

  - Encontrado o valor mais caro no brasil sem o id
  ```
